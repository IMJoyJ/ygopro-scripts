--絢嵐たる軒昂
-- 效果：
-- ①：可以从以下效果选择1个发动（这个卡名的以下效果1回合各能选择1次）。
-- ●从自己墓地把1只6星以下的「绚岚」怪兽特殊召唤。
-- ●从自己的卡组·墓地把1张「旋风」加入手卡。
-- ②：这张卡被「旋风」的效果破坏的场合才能发动。这张卡在自己的魔法与陷阱区域盖放。
local s,id,o=GetID()
-- 定义一个函数s.initial_effect(c)，用于注册卡片效果。
function s.initial_effect(c)
	-- 将卡片5318639的代码添加到该卡的CodeList中，用于追踪同名卡的使用次数。
	aux.AddCodeList(c,5318639)
	-- 创建并注册第一个效果e1：选择从墓地特殊召唤或从卡组/墓地检索“旋风”。
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))  --"发动"
	e1:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND+CATEGORY_GRAVE_ACTION)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetHintTiming(TIMING_DRAW_PHASE,TIMING_DRAW_PHASE+TIMINGS_CHECK_MONSTER+TIMING_END_PHASE)
	e1:SetTarget(s.target)
	e1:SetOperation(s.activate)
	c:RegisterEffect(e1)
	-- 创建并注册第二个效果e2：当这张卡被“旋风”的效果破坏时，将其盖放在魔法与陷阱区域。
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(id,1))  --"盖放"
	e2:SetCategory(CATEGORY_SSET)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetProperty(EFFECT_FLAG_DELAY)
	e2:SetCode(EVENT_DESTROYED)
	e2:SetCondition(s.setcon)
	e2:SetTarget(s.settg)
	e2:SetOperation(s.setop)
	c:RegisterEffect(e2)
end
-- 定义一个过滤函数s.spfilter(c,e,tp)，用于筛选6星以下的“绚岚”怪兽，且可以特殊召唤。
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x1d1) and c:IsLevelBelow(6) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
-- 定义一个过滤函数s.thfilter(c)，用于筛选卡组或墓地中的“旋风”，且可以加入手牌。
function s.thfilter(c)
	return c:IsCode(5318639) and c:IsAbleToHand()
end
-- 定义一个目标选择函数s.target(e,tp,eg,ep,ev,re,r,rp,chk)，处理玩家的选择，并设置效果标签。
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查是否有满足条件的6星以下“绚岚”怪兽在墓地，并且有空的怪兽区。
	local b1=Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		-- 检查是否已经选择了效果或者重置了该卡的效果次数限制。
		and (not e:IsCostChecked() or Duel.GetFlagEffect(tp,id)==0)
	-- 检查是否有满足条件的“旋风”在卡组或墓地。
	local b2=Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil)
		-- 检查是否已经选择了效果或者重置了该卡的效果次数限制。
		and (not e:IsCostChecked() or Duel.GetFlagEffect(tp,id+o)==0)
	if chk==0 then return b1 or b2 end
	local op=0
	if b1 or b2 then
		-- 使用aux.SelectFromOptions让玩家选择特殊召唤或检索，并设置效果标签。
		op=aux.SelectFromOptions(tp,
			{b1,aux.Stringid(id,2),1},  --"特殊召唤"
			{b2,aux.Stringid(id,3),2})  --"检索"
	end
	e:SetLabel(op)
	if op==1 then
		if e:IsCostChecked() then
			e:SetCategory(CATEGORY_SPECIAL_SUMMON)
			-- 注册一个FlagEffect，用于限制“绚岚”怪兽的特殊召唤次数。
			Duel.RegisterFlagEffect(tp,id,RESET_PHASE+PHASE_END,0,1)
		end
		-- 设置操作信息，表示将进行特殊召唤。
		Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE)
	elseif op==2 then
		if e:IsCostChecked() then
			e:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND+CATEGORY_GRAVE_ACTION)
			-- 注册一个FlagEffect，用于限制“旋风”的检索次数。
			Duel.RegisterFlagEffect(tp,id+o,RESET_PHASE+PHASE_END,0,1)
		end
		-- 设置操作信息，表示将进行检索。
		Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK+LOCATION_GRAVE)
	end
end
-- 定义一个激活函数s.activate(e,tp,eg,ep,ev,re,r,rp)，根据效果标签执行不同的操作。
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==1 then
		-- 如果怪兽区没有空位则返回
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
		-- 提示玩家选择要特殊召唤的卡片。
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)  --"请选择要特殊召唤的卡"
		-- 使用Duel.SelectMatchingCard让玩家从墓地中选择满足条件的“绚岚”怪兽。
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
		if g:GetCount()>0 then
			-- 将选中的怪兽特殊召唤到场上。
			Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
		end
	elseif e:GetLabel()==2 then
		-- 提示玩家选择要加入手牌的卡片。
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)  --"请选择要加入手牌的卡"
		-- 使用Duel.SelectMatchingCard让玩家从卡组或墓地中选择“旋风”。
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.thfilter),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
		if g:GetCount()>0 then
			-- 将选中的“旋风”加入到玩家的手牌中。
			Duel.SendtoHand(g,nil,REASON_EFFECT)
			-- 确认加入手牌的卡片。
			Duel.ConfirmCards(1-tp,g)
		end
	end
end
-- 定义一个条件函数s.setcon(e,tp,eg,ep,ev,re,r,rp)，用于判断是否满足盖放效果的触发条件。
function s.setcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsReason(REASON_EFFECT) and re:GetHandler():IsCode(5318639)
end
-- 定义一个目标选择函数s.settg(e,tp,eg,ep,ev,re,r,rp,chk)，用于设置盖放效果的目标。
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsSSetable() end
	-- 设置操作信息，表示将从墓地移除卡片。
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,e:GetHandler(),1,0,0)
end
-- 定义一个执行函数s.setop(e,tp,eg,ep,ev,re,r,rp)，用于执行盖放效果。
function s.setop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- 检查当前效果是否与连锁有关，并且不受王家长眠之谷的影响。
	if c:IsRelateToChain() and aux.NecroValleyFilter()(c) then
		-- 将卡片盖放在魔法与陷阱区域。
		Duel.SSet(tp,c)
	end
end
