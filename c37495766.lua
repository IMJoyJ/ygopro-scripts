...
local s,id,o=GetID()
-- 初始化卡片效果：注册①手卡特殊召唤、②卡组盖放救援ACE队魔陷、③对方效果致离场时破坏场上卡效果
...
-- 除外代价过滤：属于「救援ACE队」系列且可作为代价除外
function s.costfilter(c)
	return c:IsSetCard(0x18b) and c:IsAbleToRemoveAsCost()
end
-- ①效果发动代价：从自己墓地把2张「救援ACE队」卡除外
function s.sscost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查自己墓地是否存在至少2张符合条件的「救援ACE队」卡
...
-- 盖放卡片过滤：属于「救援ACE队」系列的速攻魔法或通常陷阱卡且可以盖放
function s.setfilter(c)
	return c:IsSetCard(0x18b) and c:IsSSetable()
		and (c:IsType(TYPE_QUICKPLAY) or c:GetType()==TYPE_TRAP)
end
-- ②效果目标确认：卡组存在可盖放的符合条件的卡
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查卡组是否存在可盖放的「救援ACE队」速攻魔法·通常陷阱卡
	if chk==0 then return Duel.IsExistingMatchingCard(s.setfilter,tp,LOCATION_DECK,0,1,nil) end
end
-- ②效果操作：从卡组把最多4张卡名不同的「救援ACE队」速攻魔法·通常陷阱卡在自己场上盖放
