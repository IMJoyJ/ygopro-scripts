...
local s,id,o=GetID()
-- 初始化卡片效果：注册融合召唤手续、改变对象怪兽属性效果、对方主要阶段场上/墓地融合召唤效果及墓地自身变属性效果
function s.initial_effect(c)
	c:EnableReviveLimit()
	-- 设定融合召唤手续：魔法师族怪兽＋魔法师族以外的怪兽
	...
	-- ①：以场上1只表侧表示怪兽为对象才能发动。宣言1个属性，那只怪兽变成宣言的属性。
	...
	c:RegisterEffect(e1)
	-- ②：对方主要阶段才能发动。从自己的场上·墓地把融合怪兽卡决定的包含此卡的融合素材怪兽除外，把那1只融合怪兽从额外卡组融合召唤。
	...
	c:RegisterEffect(e2)
	-- ③：这张卡在墓地存在的场合才能发动。宣言1个属性，这张卡直到回合结束时变成宣言的属性。
