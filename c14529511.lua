...
-- 效果发动条件判断函数，判断是否为融合召唤
function s.tgcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION)
end
-- 墓地检索过滤函数，判断是否为「莫忘」卡且可送去墓地
function s.filter(c)
	return c:IsSetCard(0x1a1) and c:IsAbleToGrave()
end
-- 效果发动时的处理函数，用于设置效果目标
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查是否满足发动条件：卡组/额外卡组存在3张「莫忘」卡且本回合未发动过此效果
...
-- 破坏效果中用于筛选「莫忘」怪兽的过滤函数
function s.desfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x1a1)
end
-- 破坏效果发动时的处理函数，用于设置效果目标
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	-- 检查是否满足发动条件：本回合未发动过此效果且自己场上存在「莫忘」怪兽
	if chk==0 then return Duel.GetFlagEffect(tp,id)==0
		-- 检查是否满足发动条件：对方场上存在可破坏的卡
...
-- 手牌检索过滤函数，判断是否为「冥骸府-莫忘冥府」
function s.thfilter(c)
	return c:IsCode(43338320) and c:IsAbleToHand()
end
-- 手牌检索效果发动时的处理函数，用于设置效果目标
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查是否满足发动条件：卡组中存在「冥骸府-莫忘冥府」
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil) end
	-- 设置效果处理时要操作的卡片信息为1张「冥骸府-莫忘冥府」
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
-- 手牌检索效果发动时的处理函数，用于执行效果操作
