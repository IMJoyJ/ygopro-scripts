...
local s,id,o=GetID()
-- 为鬼神 水子守命添加同调召唤手续并注册两个效果
function s.initial_effect(c)
	-- 添加需要1只调整和1只调整以外的怪兽进行同调召唤的手续
...
-- 过滤满足「艮神鬼」卡组且能送去墓地的卡片
function s.tgfilter(c)
	return c:IsSetCard(0x1e4) and c:IsAbleToGrave()
end
-- 检查是否满足①效果的发动条件：卡组存在「艮神鬼」卡，场上有卡可以送去墓地
...
-- ②效果的发动条件函数：对方怪兽效果发动时触发
function s.setcon(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and re:IsActiveType(TYPE_MONSTER)
end
-- 过滤满足盖放条件的「艮神鬼」陷阱卡（包括墓地和除外状态）
...
-- ②效果的目标选择函数：根据场上里侧表示卡数量决定是否可选择墓地陷阱卡
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if g:IsExists(Card.IsLocation,1,nil,LOCATION_GRAVE) then
		-- 设置连锁处理信息，指定将要离开墓地的卡数量为1张
		Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,g,1,0,0)
	end
end
-- ②效果的发动处理函数：将选中的卡在自己场上盖放
