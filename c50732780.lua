...
-- 设置连锁处理信息，确定将要特殊召唤的卡的类型为CATEGORY_SPECIAL_SUMMON
function c50732780.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	-- 设置当前处理的连锁的操作信息，指定目标为卡组中的一张怪兽
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
-- 检查场上是否有空位，若无则不执行；若有则提示选择并特殊召唤符合条件的怪兽
function c50732780.operation(e,tp,eg,ep,ev,re,r,rp)
	-- 判断玩家场上是否还有可用区域用于特殊召唤
