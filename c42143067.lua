...
-- 过滤手牌中满足条件的岩石族怪兽（可丢弃且卡组中有相同属性和等级的岩石族怪兽）。
...
-- 过滤卡组中满足条件的岩石族怪兽（属性和等级与丢弃的怪兽相同且可特殊召唤）。
...
-- 判断是否满足发动条件并选择丢弃的手牌，将手牌送去墓地并设置操作信息。
...
function c42143067.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc then
		-- 将选中的怪兽特殊召唤到场上，并确认对方是否能看到里侧表示的怪兽。
