...
function c94599451.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- ①：每次自己场上的表侧表示的「魔导兽」灵摆怪兽被战斗·效果破坏给这张卡放置2个魔力指示物。
	...
	c:RegisterEffect(e2)
	-- ②：1回合1次，把自己场上的魔力指示物任意数量去除才能发动。从卡组的怪兽以及自己的额外卡组的表侧表示的灵摆怪兽之中选择持有和去除数量相同等级的1只可以放置魔力指示物的怪兽加入手卡。
	...
	c:RegisterEffect(e3)
	-- ③：场上的这张卡被效果破坏的场合，可以作为代替把这张卡1个魔力指示物去除。
...
-- 放置指示物过滤条件：破坏前在场上表侧表示、由自己操控的「魔导兽」灵摆怪兽因战斗或效果被破坏
...
-- ①效果发动条件检查：被破坏的卡中存在满足条件的「魔导兽」灵摆怪兽
...
-- 检索目标过滤条件：卡组中或额外卡组表侧表示、能放置指示物且等级大于0的怪兽，且场上有对应数量指示物可去除
...
function c94599451.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 让玩家选择并声明要去除的指示物数量（决定检索怪兽等级）
	local lv=Duel.AnnounceNumber(tp,table.unpack(lvt))
	-- 从自己场上去除对应数量的魔力指示物作为Cost
	Duel.RemoveCounter(tp,1,0,0x1,lv,REASON_COST)
	e:SetLabel(lv)
	-- 设置连锁操作信息：从卡组/额外卡组检索1张卡加入手牌
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK+LOCATION_EXTRA)
end
-- 效果处理过滤条件：卡组或额外卡组表侧表示、等级与去除数量相同的可放置指示物的怪兽
...
-- ②效果处理：从卡组/额外卡组选1只对应等级的可放置指示物的怪兽加入手牌
...
-- ③效果破坏代替目标确定：因效果被破坏且此卡有指示物时询问是否代替
function c94599451.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 询问玩家是否去除1个魔力指示物以代替破坏
	return Duel.SelectEffectYesNo(tp,c,96)
end
-- ③效果破坏代替处理：从此卡去除1个魔力指示物
