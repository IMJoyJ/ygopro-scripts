...
-- 检查触发效果的卡是否因对方破坏而送去墓地，且不是因规则送去墓地，且上一个控制者是自己
...
-- 设置连锁操作信息，表明将要从自己卡组最上面送去墓地1张卡
function c41952656.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	-- 设置当前处理的连锁的操作信息，指定将要处理的卡为卡组最上面的1张卡，位置为墓地
	Duel.SetOperationInfo(0,CATEGORY_DECKDES,nil,0,tp,1)
end
-- 执行效果处理，将自己卡组最上面的1张卡送去墓地，若送去墓地的卡是水属性怪兽则抽1张卡
function c41952656.drop(e,tp,eg,ep,ev,re,r,rp)
	-- 将自己卡组最上面的1张卡送去墓地，并判断是否成功
	if Duel.DiscardDeck(tp,1,REASON_EFFECT)==1 then
		-- 获取刚刚执行的卡片操作中实际被处理的卡片组
