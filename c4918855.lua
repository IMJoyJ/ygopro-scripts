...
-- 处理效果发动后的操作，包括判断是否满足特殊召唤条件、执行特殊召唤及附加效果。
...
-- 判断是否满足发动无效效果的条件，包括对方怪兽效果发动、连锁可被无效且己方墓地存在同名怪兽。
...
-- 定义用于筛选特殊召唤怪兽的过滤条件，判断是否为从墓地召唤且为怪兽类型。
