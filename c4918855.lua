...
-- 定义了用于判断是否可以特殊召唤的过滤函数
function c4918855.spfilter(c,e,tp)
	return c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
-- 设置效果目标选择函数，检查对方墓地是否存在满足条件的怪兽
...
-- 处理效果发动后的特殊召唤流程，包括区域检查、对象筛选与效果赋予
function c4918855.spop1(e,tp,eg,ep,ev,re,r,rp)
	-- 获取玩家当前场上可用的怪兽区域数量
	local ct=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ct<1 then return end
	-- 从连锁信息中提取已选择的目标怪兽，并过滤出与当前效果相关的怪兽
...
function c4918855.negcon(e,tp,eg,ep,ev,re,r,rp)
	-- 检查连锁发动的效果是否为怪兽类型且可以被无效
	return re:IsActiveType(TYPE_MONSTER) and Duel.IsChainNegatable(ev)
		-- 检测自己墓地是否存在与当前发动怪兽同名的怪兽
