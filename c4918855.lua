...
-- 设置效果目标选择函数，用于在对方墓地中选择满足条件的怪兽作为目标。
...
-- 处理效果发动后的操作，包括判断是否满足特殊召唤条件、选择要特殊召唤的卡并执行特殊召唤。
function c4918855.spop1(e,tp,eg,ep,ev,re,r,rp)
	-- 获取玩家当前场上可用的怪兽区域数量。
	local ct=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ct<1 then return end
	-- 从连锁信息中获取已选定的目标卡片，并筛选出与当前效果相关的卡片。
...
function c4918855.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置连锁操作信息，表明将要进行无效化操作。
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,0,0)
end
-- 处理效果发动后的操作，使当前连锁发动无效。
...
-- 处理效果发动后的操作，判断是否满足特殊召唤条件并执行特殊召唤。
