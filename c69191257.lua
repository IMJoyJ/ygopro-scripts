...
local s,id,o=GetID()
-- 初始化卡片效果：注册同调召唤手续、①特召成功卡组送墓/特召机械族调整效果、②同调召唤状态被效果破坏送墓除外墓地调整破坏卡片效果
function s.initial_effect(c)
	-- 同调召唤手续：调整+1只以上调整以外的怪兽
	aux.AddSynchroProcedure(c,nil,aux.NonTuner(nil),1)
	c:EnableReviveLimit()
	-- 这张卡特殊召唤的场合：可以从卡组把1只机械族调整送去墓地。自己场上有持有和原本等级不同等级的怪兽存在的场合，也能作为代替特殊召唤。「翠铝之机龙」的这个效果1回合只能使用1次。
...
-- 目标怪兽过滤条件：卡组中的机械族调整怪兽，且满足送去墓地或特殊召唤的条件
...
-- 代替特召判断条件：场上表侧表示且当前等级与原本等级不同的1星以上怪兽
function s.cfilter(c)
	local lv=c:GetOriginalLevel()
	return c:IsFaceup() and not c:IsLevel(lv) and c:IsLevelAbove(1)
end
-- ①效果发动准备：检查卡组是否存在满足送墓或特召条件的机械族调整
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		-- 检查自己场上是否存在持有和原本等级不同等级的怪兽
...
-- ②效果发动条件：此卡为同调召唤状态且在怪兽区被效果破坏送去墓地
...
-- Cost过滤条件：墓地的调整怪兽且可除外
function s.costfilter(c)
	return c:IsType(TYPE_TUNER) and c:IsAbleToRemoveAsCost()
end
-- ②效果发动Cost：从自己墓地将任意数量的调整怪兽除外，并记录除外数量
function s.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- Cost检查：墓地是否存在至少1只可除外的调整怪兽
	...
	-- 将选中的调整怪兽表侧表示除外作为Cost，并获取实际除外数量
	local ct=Duel.Remove(g,POS_FACEUP,REASON_COST)
	e:SetLabel(ct)
end
-- ②效果发动准备：选择与除外数量相同数量的场上的卡为对象，设置破坏操作信息
