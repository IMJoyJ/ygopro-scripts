...
-- 发动时的费用处理，将此卡送去墓地作为费用，并记录其上的武士道指示物数量。
...
-- 过滤函数，用于选择满足等级要求、种族为「六武众」或「紫炎」、具有效果且可特殊召唤的怪兽。
...
-- 设置特殊召唤效果的目标，检查场上是否有足够的空间以及卡组中是否存在符合条件的怪兽。
function c47436247.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置连锁操作信息，表明此效果将特殊召唤一张怪兽。
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
-- 执行特殊召唤操作，从卡组选择符合条件的怪兽并将其特殊召唤到场上。
