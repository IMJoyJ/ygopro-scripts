...
-- 设置发动时的标签为100，表示已支付费用。
function c458748.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(100)
	return true
end
-- 过滤场上满足条件的「召唤兽」怪兽，用于判断是否可以发动效果。
function c458748.filter1(c,e,tp)
	-- 检查是否存在满足条件的额外卡组中的「召唤兽」怪兽作为融合召唤目标。
