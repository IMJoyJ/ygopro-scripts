...
-- 效果处理时判断是否满足条件，检查玩家是否可以抽2张卡并是否存在己方场上的怪兽作为对象
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- 设置操作信息，将要破坏的卡加入操作信息中
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
	-- 设置操作信息，将要抽2张卡加入操作信息中
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,2)
end
-- 处理①效果的主要逻辑，包括破坏对象怪兽、抽卡和是否改变对方怪兽表示形式
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	-- 获取连锁中的目标玩家和目标参数（即抽卡数量）
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	-- 获取与当前连锁相关的对象怪兽组并筛选出怪兽类型
...
-- 设置②效果的目标信息，准备将此卡加入手牌
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToHand() end
	-- 设置操作信息，将要加入手牌的卡加入操作信息中
