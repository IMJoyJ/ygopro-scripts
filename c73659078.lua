...
function c73659078.initial_effect(c)
	...
	c:EnableReviveLimit()
	-- ①：1回合1次，把这张卡1个超量素材去除才能发动。手卡的水属性怪兽任意数量给对方观看，给观看的数量的冰指示物给场上表侧表示存在的怪兽放置。
	...
	c:RegisterEffect(e1)
	-- ②：只要这张卡在场上表侧表示存在，水属性以外的场上的怪兽的攻击力下降场上的冰指示物数量×200。
...
-- ①效果发动条件检查：手牌存在可展示的水属性怪兽且场上存在可放置冰指示物的怪兽
...
-- ①效果处理：展示手牌水属性怪兽，并按展示数量分步给场上表侧表示怪兽放置冰指示物
function c73659078.operation(e,tp,eg,ep,ev,re,r,rp)
	-- 获取场上所有可以放置冰指示物的表侧表示怪兽组
...
-- 计算攻击力降低数值：全场冰指示物数量×(-200)
function c73659078.atkval(e,c)
	-- 返回场上冰指示物总数×(-200)的降低数值
