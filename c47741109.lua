...
function c47741109.initial_effect(c)
	-- 手卡·场上的这张卡被送去墓地的场合，可以把自己卡组最上面的卡翻开。此外，卡组的这张卡被卡的效果翻开送去墓地的场合，可以把自己卡组最上面的卡翻开。
...
-- 判断此卡是否从手牌或场上被送去墓地，或者从卡组因翻开效果被送去墓地
...
-- 执行翻开卡组最上方1张卡并根据其种族决定处理方式（送去墓地或放回卡组底端）
function c47741109.operation(e,tp,eg,ep,ev,re,r,rp)
	-- 检查玩家是否可以将卡组最上方的1张卡送去墓地
	if not Duel.IsPlayerCanDiscardDeck(tp,1) then return end
	-- 确认玩家卡组最上方的1张卡
	Duel.ConfirmDecktop(tp,1)
	-- 获取玩家卡组最上方的1张卡组成的Group
	local g=Duel.GetDecktopGroup(tp,1)
	local tc=g:GetFirst()
	if tc:IsRace(RACE_PLANT) then
		-- 禁用接下来的操作是否需要洗切卡组或手卡的检查
