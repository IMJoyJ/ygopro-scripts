...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- 效果无效效果：自己场上的「奏悦机组」怪兽为对象的卡的效果由对方发动时，可以从自己墓地把这张卡除外；那个效果无效
...
-- 特殊召唤过滤条件：满足种族为奏悦机组、可以特殊召唤、场上不存在同名卡、且满足位置和怪兽区空位条件
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x1cf) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		...
		and (c:IsLocation(LOCATION_DECK+LOCATION_GRAVE) and Duel.GetMZoneCount(tp)>0
			-- 若在额外卡组，则检查是否有额外卡组特殊召唤的空位
			or c:IsLocation(LOCATION_EXTRA) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0)
end
-- 效果发动时的处理函数，判断是否存在满足条件的卡片
...
-- 发动效果的处理函数，选择并特殊召唤符合条件的怪兽，并设置后续限制
...
-- 限制额外卡组特殊召唤的条件：必须是机械族且攻击力在1500以下
...
-- 连锁时点效果的发动条件：目标卡具有卡片目标属性，且目标卡组中存在己方场上奏悦机组怪兽
