...
function c51788412.initial_effect(c)
	c:EnableReviveLimit()
	-- 为这张卡添加融合召唤手续：以4只满足「古代的机械」字段的怪兽作为融合素材，可进行融合召唤。
	...
	-- 将特殊召唤条件设为：只有通过融合召唤才能特殊召唤这张卡（即限制其他特殊召唤方式）。
...
-- 免疫效果的过滤函数：若发动/适用的效果是魔法卡或陷阱卡，则返回true，即只免疫魔法·陷阱卡的效果。
function c51788412.efilter(e,te)
	return te:IsActiveType(TYPE_SPELL+TYPE_TRAP)
end
-- 发动限制的条件：当前阶段处于战斗阶段开始到战斗阶段结束之间（即战斗阶段中）时，该限制效果适用。
function c51788412.actcon(e)
	-- 获取当前战斗阶段并存入局部变量ph。
	local ph=Duel.GetCurrentPhase()
	return ph>=PHASE_BATTLE_START and ph<=PHASE_BATTLE
end
-- 效果限制函数：若对方发动的效果是怪兽效果，则返回true，即禁止对方在战斗阶段中发动怪兽效果。
