...
-- 设置连锁操作信息，表示将要从额外卡组特殊召唤一只融合怪兽
function c51858200.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=tp
		-- 获取玩家当前可用的融合素材，并筛选出属于「捕食植物」系列的
		...
		if not res then
			...
			if ce~=nil then
				...
				-- 如果存在连锁融合素材效果，则再次检查是否有符合条件的融合怪兽
...
-- 处理融合召唤效果，包括选择融合怪兽、选择融合素材并进行特殊召唤
function c51858200.spop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	-- 获取玩家当前可用的融合素材，并筛选出属于「捕食植物」系列且未被此效果免疫的
