...
function c65187687.initial_effect(c)
	-- 为当前卡添加同调召唤手续，要求调整怪兽作为素材，并且必须包含不死族怪兽。
	...
	c:RegisterEffect(e1)
	-- 创建第二个效果：场上怪兽效果无效。设置效果描述、类别（无效化）、类型（场地型触发）、触发条件（特殊召唤成功）、生效范围（怪兽区域）、属性（延迟、对象）和次数限制。
...
-- 定义第一个效果的目标选择函数。优先从场上和墓地选择对方控制的、满足rmfilter条件的怪兽作为目标。
function c65187687.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if g:GetFirst():IsLocation(LOCATION_GRAVE) then
		...
		-- 设置操作信息，表示当前连锁的操作是移除对方的怪兽（当目标在墓地时）。
		Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,1,0,0)
	end
end
-- 定义第一个效果的操作函数。获取目标卡片，如果目标与效果相关，则将其从场上移除。
...
-- 定义第二个效果的条件判断函数。如果存在从墓地特殊召唤的怪兽且当前卡不是触发者，则返回true。
...
function c65187687.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- 选择场上满足aux.NegateEffectMonsterFilter条件的怪兽作为目标。
	Duel.SelectTarget(tp,aux.NegateEffectMonsterFilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
-- 定义第二个效果的操作函数。获取当前卡的控制者和目标卡片，如果目标表侧表示且与效果相关，则使其相关的连锁无效化，并赋予其不能被激活的效果和不能使用效果的限制。
