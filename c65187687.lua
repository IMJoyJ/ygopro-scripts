...
function c65187687.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- 效果②：己方怪兽区域存在时，对方从墓地特殊召唤怪兽时发动，使该怪兽效果无效直到回合结束
...
-- 效果①的目标选择函数，选择对方场上或墓地的1只满足条件的怪兽作为目标
...
-- 判断是否从墓地特殊召唤的过滤函数，要求是墓地召唤且为怪兽类型
...
-- 效果②的目标选择函数，选择场上1只满足条件的效果怪兽作为目标
