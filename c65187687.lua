...
function c65187687.initial_effect(c)
	-- 添加同调召唤手续，要求1只不死族调整和1只调整以外的怪兽
	aux.AddSynchroProcedure(c,c65187687.synfilter,aux.NonTuner(nil),1)
	c:EnableReviveLimit()
	-- 效果①：这张卡特殊召唤成功的场合，以对方的场上·墓地1只怪兽为对象才能发动。那只怪兽除外。
	...
	c:RegisterEffect(e1)
	-- 效果②：这张卡已在怪兽区域存在的状态，从墓地有怪兽特殊召唤的场合，以这张卡以外的场上1只表侧表示怪兽为对象才能发动。那只怪兽的效果直到回合结束时无效。
...
-- 除外效果的目标过滤器，要求为怪兽且可被除外
function c65187687.rmfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToRemove()
end
-- 效果①的处理函数，设置目标选择和操作信息
