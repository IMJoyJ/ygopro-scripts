...
function c52575195.initial_effect(c)
	-- 自己场上的兽战士族·光属性怪兽1只成为对方的卡的效果的对象时或者成为对方怪兽的攻击对象时才能发动。
...
-- 判断连锁是否由对方发动且为取对象效果，同时确认目标卡片是否为己方场上1只光属性兽战士族怪兽。
...
-- 判断是否为对方怪兽攻击己方怪兽的情况，并确认该怪兽为光属性兽战士族。
function c52575195.tgcon2(e,tp,eg,ep,ev,re,r,rp)
	-- 排除自己回合发动的条件，防止自己攻击自己时触发效果。
...
-- 检查是否满足发动条件，包括目标怪兽可除外、己方场上存在召唤空间以及此卡未在连锁中。
function c52575195.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=e:GetLabelObject()
	-- 判断目标怪兽是否可以除外且己方有足够召唤空间。
