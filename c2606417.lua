...
local s,id,o=GetID()
-- 初始化卡片效果，注册时间黑魔术师卡号，允许放置指示物，设置最多6个指示物，创建永续发动效果，创建准备阶段触发的指示物放置效果，创建攻击伤害计算时和对方怪兽效果发动时的投掷硬币效果
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	-- 攻击伤害计算时触发的投掷硬币效果，用于猜测硬币正反面并特殊召唤怪兽、使目标怪兽攻击力变为0并无效化其效果
	...
	c:RegisterEffect(e3)
	-- 对方怪兽效果发动时触发的投掷硬币效果，用于猜测硬币正反面并特殊召唤怪兽、使目标怪兽攻击力变为0并无效化其效果
...
-- 指示物放置效果的操作函数，掷骰子决定放置指示物数量，并处理最大6个限制
...
-- 对方怪兽效果发动时触发效果的条件函数，检查效果发动者是否为对方控制且满足mfilter条件
...
-- 对方怪兽效果发动时触发效果的目标函数，检查目标怪兽是否可以成为效果对象且满足特殊召唤条件
...
-- 投掷硬币效果的操作函数，处理猜测结果并执行特殊召唤、使目标怪兽攻击力变为0和无效化其效果
