...
-- 对方怪兽效果发动时触发效果的目标函数，检查目标是否可以成为效果对象、是否有足够召唤位置和特殊召唤卡牌
...
-- 投掷硬币效果的处理函数，根据猜中与否决定是否特殊召唤并使目标怪兽无效化和攻击力归零
