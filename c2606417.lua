...
-- 攻击伤害计算时触发效果的条件函数，判断攻击怪兽是否为对方控制且满足处理条件
function s.atkcon(e,tp,eg,ep,ev,re,r,rp)
	-- 获取当前攻击怪兽
	local a=Duel.GetAttacker()
	return a:IsControler(1-tp) and s.mfilter(a)
end
-- 对方怪兽效果发动时触发效果的条件函数，判断发动者是否为对方且满足处理条件
...
-- 攻击伤害计算时触发效果的目标函数，检查目标怪兽是否可以成为效果对象并满足特殊召唤条件
...
-- 对方怪兽效果发动时触发效果的目标函数，检查目标怪兽是否可以成为效果对象并满足特殊召唤条件
...
-- 投掷硬币效果的操作函数，进行猜硬币并特殊召唤时间黑魔术师怪兽，同时使目标怪兽攻击力归零并无效化
