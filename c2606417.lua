...
-- 对方怪兽效果发动时触发效果的条件函数，判断是否为对方怪兽发动的效果且目标满足mfilter条件
...
-- 筛选卡组中可特殊召唤的时间黑魔术师相关怪兽的过滤函数
function s.spfilter(c,e,tp)
	-- 判断卡片是否记载着时间黑魔术师（卡号40235813）且可以特殊召唤
	return aux.IsCodeListed(c,40235813) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
-- 攻击伤害计算时触发效果的目标函数，检查目标怪兽是否能成为效果对象、场上是否有空位、卡组是否存在可特殊召唤的怪兽
...
-- 对方怪兽效果发动时触发效果的目标函数，检查目标怪兽是否能成为效果对象、场上是否有空位、卡组是否存在可特殊召唤的怪兽
