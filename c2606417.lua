...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- 准备阶段触发的指示物放置效果，属于诱发即时效果，作用于场地魔法区域，每次只能发动一次，目标为自身，操作为投掷骰子并放置相应数量指示物
	...
	c:RegisterEffect(e2)
	-- 攻击伤害计算时触发的投掷硬币效果，属于快速连锁效果，作用于场地魔法区域，每次只能发动一次，目标为攻击怪兽，操作为投掷硬币并根据结果特殊召唤时间黑魔术师怪兽、使对象怪兽攻击力变为0且效果无效化
	...
	c:RegisterEffect(e3)
	-- 对方怪兽效果发动时触发的投掷硬币效果，属于快速连锁效果，作用于场地魔法区域，每次只能发动一次，目标为发动效果的怪兽，操作为投掷硬币并根据结果特殊召唤时间黑魔术师怪兽、使对象怪兽攻击力变为0且效果无效化
...
-- 指示物放置效果的操作函数，执行骰子投掷和指示物添加逻辑，若超过上限则调整数量
...
-- 对方怪兽效果发动时触发效果的条件函数，判断效果发动者是否为对方控制且为怪兽类型且满足目标条件
...
-- 对方怪兽效果发动时触发效果的目标函数，设置目标为发动效果的怪兽，检查是否有足够召唤位置和特殊召唤卡牌
...
-- 投掷硬币效果的操作函数，执行硬币投掷并根据结果处理特殊召唤和对象怪兽效果
