...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- 准备阶段触发的放置指示物效果，属于诱发即时效果，作用是掷骰子并放置相应数量的指示物
	...
	c:RegisterEffect(e2)
	-- 攻击伤害计算时触发的投掷硬币效果，属于快速效果，作用是消耗一个指示物并进行硬币投掷，猜中则特殊召唤时间黑魔术师怪兽并使目标怪兽攻击力变为0且效果无效
	...
	c:RegisterEffect(e3)
	-- 对方怪兽效果发动时触发的投掷硬币效果，属于快速效果，作用与第4条相同
...
-- 放置指示物效果的目标函数，用于检查是否满足发动条件
function s.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
end
-- 放置指示物效果的操作函数，执行掷骰子并根据结果决定是否放置指示物
...
-- 对方怪兽效果发动时触发效果的目标函数，检查目标怪兽是否可成为效果对象并满足特殊召唤条件
...
-- 投掷硬币效果的操作函数，执行硬币投掷并根据结果决定是否特殊召唤怪兽及使目标怪兽效果无效
