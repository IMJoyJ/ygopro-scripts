...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- 准备阶段触发的指示物放置效果，属于诱发即时效果，作用于场地魔法区域，每次只能发动一次，目标为自身，操作为投掷骰子并放置相应数量的指示物
	...
	c:RegisterEffect(e2)
	-- 攻击伤害计算时触发的投掷硬币效果，属于快速效果，作用于场地魔法区域，每次只能发动一次，目标为攻击怪兽，操作为投掷硬币并根据结果特殊召唤时间黑魔术师相关怪兽、使对象怪兽攻击力变为0且效果无效化
	...
	c:RegisterEffect(e3)
	-- 对方怪兽效果发动时触发的投掷硬币效果，属于快速效果，作用于场地魔法区域，每次只能发动一次，目标为发动效果的怪兽，操作为投掷硬币并根据结果特殊召唤时间黑魔术师相关怪兽、使对象怪兽攻击力变为0且效果无效化
...
-- 指示物放置效果的目标函数，用于判断是否满足触发条件，此处无实际判断逻辑
function s.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
end
-- 指示物放置效果的操作函数，执行骰子投掷和指示物添加逻辑，若总数量超过6则限制为最多6个
...
-- 判断怪兽是否在场上且表侧表示且为怪兽类型，且攻击力不为0或未被无效化
...
-- 筛选卡组中时间黑魔术师相关的怪兽用于特殊召唤
function s.spfilter(c,e,tp)
	-- 判断卡片是否记载着时间黑魔术师（40235813）这张卡且可以特殊召唤
	return aux.IsCodeListed(c,40235813) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
-- 攻击伤害计算时触发效果的目标函数，判断攻击怪兽是否可成为效果对象、场上是否有空位、卡组中是否存在时间黑魔术师相关怪兽
...
-- 对方怪兽效果发动时触发效果的目标函数，判断发动效果的怪兽是否在场且可成为效果对象、场上是否有空位、卡组中是否存在时间黑魔术师相关怪兽
...
-- 投掷硬币效果的操作函数，执行硬币投掷并根据结果处理特殊召唤和对象怪兽效果无效化
