...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	-- 攻击伤害计算时触发的投掷硬币效果，属于快速效果，作用于场地魔法区域，每次只能发动一次，目标为攻击怪兽，操作为投掷硬币并根据结果特殊召唤时间黑魔术师相关怪兽并使其攻击力变为0、效果无效化
	...
	c:RegisterEffect(e3)
	-- 对方怪兽效果发动时触发的投掷硬币效果，属于快速效果，作用于场地魔法区域，每次只能发动一次，目标为发动效果的怪兽，操作为投掷硬币并根据结果特殊召唤时间黑魔术师相关怪兽并使其攻击力变为0、效果无效化
...
-- 指示物放置效果的目标函数，用于判断是否满足发动条件，此处无实际判断逻辑
function s.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
end
-- 指示物放置效果的操作函数，执行骰子投掷并根据结果决定放置多少指示物，若超过上限则自动调整
...
-- 对方怪兽效果发动时触发效果的目标函数，判断发动怪兽是否可成为目标且满足特殊召唤条件
...
-- 投掷硬币效果的操作函数，执行硬币投掷并根据结果特殊召唤怪兽、使目标怪兽攻击力变为0、效果无效化
