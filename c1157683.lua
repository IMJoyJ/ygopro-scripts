...
-- 装备限制函数，判断当前装备卡是否只能装备给特定怪兽。
function c1157683.eqlimit(e,c)
	return c==e:GetLabelObject()
end
-- 过滤函数，用于判断场上是否有满足条件的装备卡（必须正面表示、装备着机械族怪兽且可作为费用送入墓地）。
...
-- 设置破坏效果的发动条件：检查是否有满足条件的装备卡可作为费用送入墓地。
...
-- 设置破坏效果的发动条件：检查是否已发动过此效果、对方场上是否存在可破坏的卡。
