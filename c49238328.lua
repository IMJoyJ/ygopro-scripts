...
function c49238328.initial_effect(c)
	-- ①：自己主要阶段1开始时，把自己的额外卡组3张或6张里侧的卡随机里侧除外才能发动。
...
-- 过滤满足条件的额外卡组卡片（里侧表示且可作为费用除外）
function c49238328.cfilter(c)
	return c:IsFacedown() and c:IsAbleToRemoveAsCost(POS_FACEDOWN)
end
-- 检索满足条件的额外卡组卡片，判断是否可以发动效果并选择除外3张或6张
function c49238328.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		-- 判断玩家是否可以抽1张卡且额外卡组有至少3张符合条件的卡
		return Duel.IsPlayerCanDraw(tp,1) and #g>=3
	end
	local op=0
	-- 判断玩家是否可以抽2张卡且额外卡组有至少6张符合条件的卡
