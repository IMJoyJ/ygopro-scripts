...
function c13166204.initial_effect(c)
	-- 自己场上的名字带有「我我我」的怪兽成为对方怪兽的效果的对象时才能发动。
...
-- 过滤满足条件的怪兽（表侧表示、在主要怪兽区、控制者为tp、名字带有「我我我」）
...
-- 设置效果发动后的处理流程（无效效果、破坏怪兽、计算并造成伤害）
function c13166204.activate(e,tp,eg,ep,ev,re,r,rp)
	-- 判断是否成功使效果无效并破坏对方怪兽，若成功则继续计算伤害
	if Duel.NegateEffect(ev) and re:GetHandler():IsRelateToEffect(re) and Duel.Destroy(re:GetHandler(),REASON_EFFECT)~=0 then
		...
		if b>a then a=b end
		if a>0 then
			-- 中断当前效果处理，使后续伤害处理视为错时点
			Duel.BreakEffect()
			-- 对对方造成伤害，伤害值为被破坏怪兽攻击力与守备力中较高者
