--獄神影機－ゼグレド
-- 效果：
-- ←1 【灵摆】 1→
-- 这个卡名的灵摆效果1回合只能使用1次。
-- ①：自己主要阶段才能发动。这张卡和自己的手卡·场上（表侧表示）1只「狱神」怪兽或「终刻」怪兽破坏。那之后，可以把场上1张卡破坏。
-- 【怪兽效果】
-- 这个卡名在规则上也当作「终刻」卡使用。这个卡名的①②的怪兽效果1回合各能使用1次。
-- ①：从卡组上面把3张卡里侧除外才能发动。这张卡破坏，从额外卡组把1只「坏狱神 朱庇特」当作超量召唤作特殊召唤。那之后，可以选自己1张手卡作为那超量素材。
-- ②：这张卡表侧加入额外卡组的场合才能发动。从卡组把1张「狱神」魔法·陷阱卡或「终刻」魔法·陷阱卡加入手卡。
local s,id,o=GetID()
-- 初始化函数：注册本卡拥有的所有效果，包括灵摆效果①（破坏本卡及己方手卡/场上的狱神或终刻怪兽，再选场上1张卡破坏），以及怪兽效果①（起动效果：里侧除外卡组顶3张卡，破坏自身并从额外卡组特殊召唤「坏狱神 朱庇特」，可追加叠放手牌素材）和怪兽效果②（加入额外卡组时的诱发效果，检索「狱神」或「终刻」魔法/陷阱卡）
function s.initial_effect(c)
	-- 记录本卡在规则上记述有「坏狱神 朱庇特」的卡名
	aux.AddCodeList(c,68231287)
	-- 为卡片添加灵摆怪兽的基本属性（包括灵摆刻度和灵摆卡的发动）
	aux.EnablePendulumAttribute(c)
	-- ①：自己主要阶段才能发动。这张卡和自己的手卡·场上（表侧表示）1只「狱神」怪兽或「终刻」怪兽破坏。那之后，可以把场上1张卡破坏。
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))  --"破坏效果"
	e1:SetCategory(CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_PZONE)
	e1:SetCountLimit(1,id)
	e1:SetTarget(s.destg)
	e1:SetOperation(s.desop)
	c:RegisterEffect(e1)
	-- ①：从卡组上面把3张卡里侧除外才能发动。这张卡破坏，从额外卡组把1只「坏狱神 朱庇特」当作超量召唤作特殊召唤。那之后，可以选自己1张手卡作为那超量素材。
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(id,1))  --"特殊召唤"
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DESTROY)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1,id+o)
	e2:SetCost(s.spcost)
	e2:SetTarget(s.sptg)
	e2:SetOperation(s.spop)
	c:RegisterEffect(e2)
	-- ②：这张卡表侧加入额外卡组的场合才能发动。从卡组把1张「狱神」魔法·陷阱卡或「终刻」魔法·陷阱卡加入手卡。
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(id,2))  --"检索效果"
	e3:SetCategory(CATEGORY_SEARCH+CATEGORY_TOHAND)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_TO_DECK)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCountLimit(1,id+o*2)
	e3:SetCondition(s.thcon)
	e3:SetTarget(s.thtg)
	e3:SetOperation(s.thop)
	c:RegisterEffect(e3)
end
-- 过滤条件：检查卡片是否是自己场上表侧表示或自己手牌中的「狱神」或「终刻」怪兽
function s.desfilter(c)
	return c:IsFaceupEx() and c:IsSetCard(0x1ce,0x1d2)
		and c:IsType(TYPE_MONSTER)
end
-- 过滤条件：检查卡片是否为手牌中且已公开的「狱神」或「终刻」怪兽
function s.hdesfilter(c)
	return c:IsSetCard(0x1ce,0x1d2) and c:IsType(TYPE_MONSTER) and c:IsPublic()
end
-- 灵摆效果①的发动准备与合法性检查：检查己方手牌或场上是否存在除自身外可供破坏的「狱神」或「终刻」怪兽，并根据手牌中是否有非公开卡片来预设要破坏的卡片数量并注册破坏操作信息
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	-- 在效果发动时，检查除自己以外，自己的手牌或场上是否至少存在1只符合条件的「狱神」或「终刻」怪兽
	if chk==0 then return Duel.IsExistingMatchingCard(s.desfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,c) end
	-- 获取除了自身以外，自己手牌或场上所有符合条件的「狱神」或「终刻」怪兽
	local g=Duel.GetMatchingGroup(s.desfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,c)
	g:AddCard(c)
	-- 检查手牌中是否有尚未公开信息的卡片
	if Duel.IsExistingMatchingCard(aux.NOT(Card.IsPublic),tp,LOCATION_HAND,0,1,nil)
		-- 或检查手牌中是否有符合破坏条件且已经公开信息的怪兽
		or Duel.IsExistingMatchingCard(s.hdesfilter,tp,LOCATION_HAND,0,1,nil) then
		-- 若存在非公开手牌信息，为了隐藏效果意图，将预期的破坏操作信息设置为仅破坏自身（1张卡）
		Duel.SetOperationInfo(0,CATEGORY_DESTROY,c,1,0,0)
	else
		-- 若手牌信息全部公开，则确切设置破坏操作信息为破坏自身和所选的怪兽（共2张卡）
		Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,2,0,0)
	end
end
-- 灵摆效果①的效果处理：玩家选择1只手牌或场上表侧表示的「狱神」或「终刻」怪兽，将其与本卡一起破坏。如果两张卡均成功破坏，可选择是否追加破坏场上的1张卡。
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToChain() then return end
	-- 提示玩家选择要破坏的卡片
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)  --"请选择要破坏的卡"
	-- 让玩家从自己的手卡或场上选择1只除自身以外的「狱神」或「终刻」怪兽
	local g=Duel.SelectMatchingCard(tp,s.desfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,1,c)
	if #g>0 then
		if g:IsExists(Card.IsLocation,1,nil,LOCATION_MZONE) then
			-- 手动在场上显示选择的卡片被选为效果对象的动画
			Duel.HintSelection(g)
		end
		g:AddCard(c)
		-- 将选择的怪兽与灵摆区域的本卡一起破坏，并检查是否成功破坏了2张卡
		if Duel.Destroy(g,REASON_EFFECT)==2
			-- 且检查场上是否还存在其他可以被破坏的卡片
			and Duel.IsExistingMatchingCard(aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
			-- 且让玩家选择是否将场上1张卡破坏
			and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then  --"是否把场上的卡破坏？"
			-- 中断当前效果处理，使后续的破坏操作与之前的破坏不视为同时处理
			Duel.BreakEffect()
			-- 提示玩家选择要破坏的卡片
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)  --"请选择要破坏的卡"
			-- 让玩家在场上任意选择1张卡片
			local dg=Duel.SelectMatchingCard(tp,aux.TRUE,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
			-- 手动在场上显示选择的卡片被选为效果对象的动画
			Duel.HintSelection(dg)
			-- 将选择的那张卡片破坏
			Duel.Destroy(dg,REASON_EFFECT)
		end
	end
end
-- 过滤条件：检查墓地中是否存在「绝境的狱神域-威利亚」并可以除外作为代替代价
function s.costfilter(c,e,tp)
	return e:GetHandler():IsSetCard(0x1ce) and c:IsAbleToRemove() and c:IsHasEffect(99311889,tp)
end
-- 怪兽效果①的发动代价检查：检查是否能里侧除外卡组最上方3张卡，或者墓地中是否存在符合代替除外代价的卡片
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 获取自己卡组最上方的3张卡片
	local g=Duel.GetDecktopGroup(tp,3)
	if chk==0 then return g:FilterCount(Card.IsAbleToRemoveAsCost,nil,POS_FACEDOWN)==3
		-- 且检查卡组剩余卡片数量是否在3张以上
		and Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)>=3
		-- 或者检查墓地中是否存在可以代替除外代价的卡片
		or Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	if g:FilterCount(Card.IsAbleToRemoveAsCost,nil,POS_FACEDOWN)==3
		-- 判断是否能够正常除外卡组上方的3张卡
		and Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)>=3
		-- 并且满足无法执行墓地代替除外条件
		and (not Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp)
		-- 或者询问玩家代替除外时，玩家选择不使用代替效果，从而进行正常里侧除外代价支付
		or not Duel.SelectYesNo(tp,aux.Stringid(99311889,1))) then  --"是否作为代替把「绝境的狱神域-威利亚」除外？"
		-- 禁用接下来的洗牌检测，防止里侧除外卡组顶端卡片时触发不必要的洗牌
		Duel.DisableShuffleCheck()
		-- 将卡组最上方的3张卡里侧除外，作为效果发动的代价
		Duel.Remove(g,POS_FACEDOWN,REASON_COST)
	else
		-- 提示玩家选择要除外的卡片
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)  --"请选择要除外的卡"
		-- 让玩家从墓地选择1张作为代替代价的卡片
		local sg=Duel.SelectMatchingCard(tp,s.costfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
		local tc=sg:GetFirst()
		local te=tc:IsHasEffect(99311889,tp)
		if te then
			te:UseCountLimit(tp)
			-- 将该墓地代替卡片表侧除外，并适用代替发动的代价
			Duel.Remove(tc,POS_FACEUP,REASON_COST+REASON_REPLACE)
		end
	end
end
-- 过滤条件：寻找额外卡组的「坏狱神 朱庇特」，判定其是否能以超量召唤形式被特殊召唤，并判断额外怪兽区域是否有可用格子
function s.spfilter(c,e,tp,mc)
	return c:IsType(TYPE_XYZ) and c:IsCode(68231287)
		-- 且判定其是否能以超量召唤形式被特殊召唤，且场上有格子能供额外怪兽出场
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false) and Duel.GetLocationCountFromEx(tp,tp,mc,c)>0
end
-- 怪兽效果①的发动准备与合法性检查：需满足超量素材的限制，且额外卡组中必须有可特殊召唤的「坏狱神 朱庇特」
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	-- 在效果发动时，检查是否满足玩家受到的关于超量素材限制的约束
	if chk==0 then return aux.MustMaterialCheck(nil,tp,EFFECT_MUST_BE_XMATERIAL)
		-- 且检查额外卡组中是否有可以特殊召唤的「坏狱神 朱庇特」
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c) end
	-- 注册破坏本卡的操作信息
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,e:GetHandler(),1,0,0)
	-- 注册从额外卡组特殊召唤1只怪兽的操作信息
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
-- 过滤条件：检查卡片是否可作为超量素材进行叠放
function s.matfilter(c)
	return c:IsCanOverlay()
end
-- 怪兽效果①的效果处理：破坏自身，从额外卡组特殊召唤「坏狱神 朱庇特」作为超量召唤，之后玩家可选择是否将手卡的1张卡作为该怪兽的超量素材
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- 检查本卡是否还存在于连锁中，并将其破坏。只有成功破坏自身才继续处理后续效果
	if not c:IsRelateToChain() or Duel.Destroy(c,REASON_EFFECT)==0 then return end
	-- 再次检查特殊召唤时是否满足玩家受到的关于超量素材限制的约束
	if not aux.MustMaterialCheck(nil,tp,EFFECT_MUST_BE_XMATERIAL) then return end
	-- 提示玩家选择要特殊召唤的怪兽
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)  --"请选择要特殊召唤的卡"
	-- 让玩家从额外卡组选择1只符合召唤条件的「坏狱神 朱庇特」
	local g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,nil)
	local tc=g:GetFirst()
	if not tc then return end
	tc:SetMaterial(nil)
	-- 将所选的怪兽以超量召唤形式在自己场上特殊召唤，并检查是否召唤成功
	if Duel.SpecialSummon(tc,SUMMON_TYPE_XYZ,tp,tp,false,false,POS_FACEUP)~=0 then
		tc:CompleteProcedure()
		-- 检查自己手牌中是否存在可以作为超量素材的卡
		if Duel.IsExistingMatchingCard(s.matfilter,tp,LOCATION_HAND,0,1,nil)
			-- 并询问玩家是否选择1张手卡作为超量素材
			and Duel.SelectYesNo(tp,aux.Stringid(id,4)) then  --"是否把手卡作为超量素材？"
			-- 中断当前效果处理，使后续叠放素材的操作不与特殊召唤视为同时处理
			Duel.BreakEffect()
			-- 提示玩家选择要作为超量素材的卡片
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)  --"请选择要作为超量素材的卡"
			-- 让玩家从手牌中选择1张作为超量素材的卡片
			local mg=Duel.SelectMatchingCard(tp,s.matfilter,tp,LOCATION_HAND,0,1,1,nil)
			if mg:GetCount()>0 then
				-- 将选择的手牌作为超量素材叠放到特殊召唤的超量怪兽下
				Duel.Overlay(tc,mg)
			end
		end
	end
end
-- 诱发效果的发动条件：本卡表侧表示加入额外卡组的场合才能发动
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsLocation(LOCATION_EXTRA)
		and c:IsFaceup()
end
-- 检索过滤器：在卡组中寻找「狱神」或「终刻」魔法·陷阱卡且能加入手牌
function s.thfilter(c)
	return c:IsSetCard(0x1ce,0x1d2) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToHand()
end
-- 怪兽效果②的发动准备与合法性检查：检查卡组中是否存在可检索的「狱神」或「终刻」魔陷，并注册检索的操作信息
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 在效果发动时，检查卡组中是否存在符合检索条件的魔法或陷阱卡
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil) end
	-- 设置操作信息：将卡组中的1张魔法或陷阱卡加入手牌
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
-- 怪兽效果②的效果处理：从卡组检索1张「狱神」或「终刻」魔法·陷阱卡加入手牌
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	-- 提示玩家选择要加入手牌的卡片
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)  --"请选择要加入手牌的卡"
	-- 让玩家从卡组中选择1张符合检索条件的「狱神」或「终刻」魔法或陷阱卡
	local g=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		-- 将所选的卡片加入玩家手牌
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		-- 向对方确认加入手牌的卡片
		Duel.ConfirmCards(1-tp,g)
	end
end
