...
-- 判定目标怪兽是否为灵魂怪兽，仅允许灵魂怪兽装备此卡。
function c1200843.eqlimit(e,c)
	return c:IsType(TYPE_SPIRIT)
end
-- 过滤条件：怪兽须为表侧表示且是灵魂怪兽，用于选择装备对象。
function c1200843.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_SPIRIT)
end
-- 发动时选择场上1只表侧表示的灵魂怪兽作为装备对象，若不存在合法对象则不能发动。
function c1200843.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- 在效果发动时检查是否存在表侧表示的灵魂怪兽可以作为装备对象，若不存在则不能发动。
