...
-- 判断是否满足条件，即被破坏的怪兽是「战吼」怪兽且为正面表示、在主要阶段离开场上的怪兽，并且是由对方效果导致的离开。
...
-- 检查当前是否处于主要阶段1或主要阶段2，并且有符合条件的怪兽因对方效果离开场上。
...
-- 设置发动时的目标，即对方场上的任意卡，用于连锁信息记录。
function c46660187.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 判断是否满足发动条件，即对方场上是否存在至少一张卡。
	if chk==0 then return Duel.IsExistingMatchingCard(aux.TRUE,tp,0,LOCATION_ONFIELD,1,nil) end
	-- 获取对方场上的所有卡作为可能的破坏对象。
	local g=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_ONFIELD,nil)
	-- 设置当前效果处理中将要破坏的卡组和数量信息。
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
end
-- 执行效果操作，选择并破坏对方场上最多2张卡。
