...
-- 过滤出场上正面表示且位于主要怪兽区域（0-4）的「幻崩」怪兽，并且该怪兽存在至少一只其他「幻崩」怪兽在主要怪兽区域
...
-- 过滤出场上正面表示且位于主要怪兽区域（0-4）的「幻崩」怪兽
function c25163979.mvfilter3(c)
	return c:IsFaceup() and c:IsSetCard(0x112) and c:GetSequence()<5
end
-- 判断是否可以发动效果，检查是否有满足条件的「幻崩」怪兽可移动或交换位置
function c25163979.mvtg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查是否存在满足条件的「幻崩」怪兽用于位置移动
