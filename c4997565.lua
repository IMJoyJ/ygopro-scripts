...
function c4997565.initial_effect(c)
	-- 为这张卡添加超量召唤手续：需要2只等级3的怪兽作为超量素材叠放（满足“3星怪兽×2”）。
...
-- 将该卡的No.编号登记为3，用于与“No.”相关的规则判定。
aux.xyz_number[4997565]=3
-- 特殊召唤的过滤条件：对象必须是昆虫族怪兽，且可以被当前效果以表侧守备表示特殊召唤。
...
function c4997565.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- 将发动效果的那只怪兽设置为当前连锁的对象（取对象）。
	Duel.SetTargetCard(tc)
end
-- 处理②效果：先将对象怪兽的效果无效（并无效其发动的连锁），然后选择场上1只昆虫族怪兽，令其守备力上升500或变更表示形式。
function c4997565.disop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- 获取当前连锁的对象卡，即需要被无效的那只怪兽。
	local tc=Duel.GetFirstTarget()
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:IsCanBeDisabledByEffect(e) then
		-- 使与该对象怪兽相关的连锁（即它发动的效果）无效。
		...
		if Duel.IsExistingMatchingCard(c4997565.cfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) then
			...
			if opt==0 then
				...
				-- 变更所选怪兽的表示形式（根据当前表示形式切换为攻击表示或守备表示等）。
