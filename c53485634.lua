...
function c53485634.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- 注册一个合并延迟事件处理器，用于将同一时间点内发生的多个同类游戏事件合并为单次自定义事件触发，防止效果重复发动。
...
-- 定义一个过滤函数，用于筛选满足条件的被破坏怪兽，包括其位置、控制者、破坏原因、种族、等级和可特殊召唤性。
...
-- 检测是否满足发动条件，包括未受青眼精灵龙影响、场上存在空位、自身可特殊召唤且目标怪兽存在。
function c53485634.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 将符合条件的被破坏怪兽设置为连锁处理的目标卡片。
	Duel.SetTargetCard(g)
	-- 设置本次连锁的操作信息，表明将特殊召唤自身。
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
	-- 设置本次连锁的操作信息，表明将从墓地特殊召唤目标怪兽。
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,g,g:GetCount(),0,0)
end
-- 定义一个过滤函数，用于筛选可参与特殊召唤的目标卡片。
...
-- 处理效果发动时的特殊召唤流程，包括自身和被破坏怪兽的特殊召唤步骤。
