...
-- 定义特殊召唤的候选条件：该怪兽具有「齿轮齿轮」字段，且可以被表侧守备表示特殊召唤。
...
-- 效果发动时的目标处理函数：检查自己场上是否有可用怪兽区，且手卡·墓地是否存在满足条件的「齿轮齿轮」怪兽；若满足，则登记特殊召唤的操作信息。
function c1045143.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 在发动时（chk==0）确认自己场上存在可用的主要怪兽区空格。
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		-- 确认自己的手卡·墓地存在至少1只满足过滤条件的「齿轮齿轮」怪兽（可作为特殊召唤对象）。
		and Duel.IsExistingMatchingCard(c1045143.filter,tp,LOCATION_GRAVE+LOCATION_HAND,0,1,nil,e,tp) end
	-- 设定本次连锁的操作信息：该效果为从手卡·墓地特殊召唤1只怪兽。
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE+LOCATION_HAND)
end
-- 效果处理时的操作函数：若场上仍有空位，则选择1只符合条件的「齿轮齿轮」怪兽并表侧守备表示特殊召唤。
function c1045143.spop(e,tp,eg,ep,ev,re,r,rp)
	-- 效果处理时再次检查自己场上是否有可用主要怪兽区，若无空位则效果不处理。
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	-- 提示玩家选择要特殊召唤的卡片。
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)  --"请选择要特殊召唤的卡"
	-- 从自己的手卡·墓地选择1只满足条件且不受「王家长眠之谷」效果影响的「齿轮齿轮」怪兽。
