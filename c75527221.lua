...
local s,id,o=GetID()
-- 初始化卡片效果：注册①卡片发动时将对方怪兽送去墓地效果、②墓地除外自身将双方场上表侧表示卡送去墓地效果
function s.initial_effect(c)
	-- 注册卡片记述列表：记述「活死人的呼声」（97077563）
	...
	c:RegisterEffect(e1)
	-- 自己场上有不死族怪兽存在的场合：可以从自己墓地把这张卡除外，以自己以及对方场上的表侧表示卡各1张为对象才能发动。那些卡送去墓地。
...
-- ①效果发动条件：自己场上有表侧表示的不死族怪兽以及「活死人的呼声」存在
...
-- ①效果目标过滤条件：对方场上的怪兽且可以送去墓地
function s.tgfilter1(c)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
-- ①效果发动准备：选择对方场上1只怪兽为对象并设置送去墓地的操作信息
