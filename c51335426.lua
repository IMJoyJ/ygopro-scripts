...
-- 过滤满足条件的连接怪兽，包括墓地和场上的怪兽，且能回到额外卡组并可以抽卡。
function c51335426.filter(c,tp)
	-- 判断目标是否为场上的连接怪兽或墓地中的连接怪兽，并且可以特殊召唤到额外卡组，同时玩家可以抽相应数量的卡。
...
-- 那只怪兽回到持有者的额外卡组，自己从卡组抽出那个连接标记的数量。那之后，选抽出数量的手卡用喜欢的顺序回到卡组下面。
function c51335426.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=tc:GetLink()
	-- 判断目标怪兽是否有效，并将其送回额外卡组，且确保其在额外卡组中。
