...
local s,id,o=GetID()
-- 初始化效果，设置同调召唤条件并注册3个效果
function s.initial_effect(c)
	-- 添加同调召唤手续，要求1只调整和1只调整以外的怪兽
	aux.AddSynchroProcedure(c,nil,aux.NonTuner(nil),1)
	c:EnableReviveLimit()
	-- 对方场上的怪兽攻击力下降自己场上的装备魔法卡数量×400
...
-- 破坏效果的发动条件判断，检查是否有场上的卡可选且自己有装备魔法卡可送入墓地
...
-- 过滤器函数，用于判断是否为魔法与陷阱区域表侧表示的卡
function s.desfilter(c)
	return c:IsFaceup() and c:GetSequence()<5
end
-- 特殊召唤效果的发动条件判断，检查是否有足够的召唤位置和目标卡可破坏
