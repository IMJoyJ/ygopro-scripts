...
-- 判断是否满足发动条件，即当前处于伤害步骤且未计算战斗伤害，同时攻击怪兽或防守怪兽为念动力族且处于战斗状态。
function c21454943.condition(e,tp,eg,ep,ev,re,r,rp)
	-- 获取当前游戏阶段
	local phase=Duel.GetCurrentPhase()
	-- 若当前阶段不是伤害步骤或战斗伤害已计算，则无法发动效果
...
-- 判断是否满足支付基本分的条件，包括检查玩家是否有足够的基本分、该卡是否已发动过此效果、以及目标怪兽攻击力或守备力是否至少为100点。
function c21454943.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local bc=e:GetLabelObject()
	-- 检查玩家是否能支付100点基本分，并确认该卡未在本回合发动过此效果
