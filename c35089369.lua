...
function c35089369.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置效果①的处理信息：将要特殊召唤的怪兽数量设为1，位置设为卡组。
...
-- 判断是否满足效果②的发动条件：当前为对方回合且处于主要阶段或战斗阶段。
...
-- 效果②的发动时点判定：确认场上是否有「龙星」怪兽且额外卡组中存在可同调召唤的怪兽。
function c35089369.sctg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置效果②的处理信息：将要特殊召唤的怪兽数量设为1，位置设为额外卡组。
...
-- 判断是否满足效果③的发动条件：该卡作为同调素材被使用。
function c35089369.immcon(e,tp,eg,ep,ev,re,r,rp)
	return r==REASON_SYNCHRO
end
-- 效果③的处理：为使用该卡作为同调素材的同调怪兽添加不受魔法卡效果影响的效果。
