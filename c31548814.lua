...
-- 过滤满足条件的「炼狱」魔法·陷阱卡（表侧表示或在手卡且能作为代价送去墓地）
...
-- 检查是否有满足条件的「炼狱」魔法·陷阱卡并选择一张送去墓地作为发动代价
function c31548814.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检测是否满足发动条件（手卡或场上存在1张符合条件的「炼狱」魔法·陷阱卡）
...
-- 检测是否满足特殊召唤条件（场上可用位置和卡组中是否有满足等级合计为8的怪兽）
function c31548814.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		-- 获取玩家场上可用的怪兽区域数量（最多3个）
