...
-- 
local s,id,o=GetID()
-- 初始化卡片效果：注册手牌规则特召效果、处于「奏悦机组」连接区时支付1000基本分破坏对方场上卡及自身并给予伤害效果
function s.initial_effect(c)
	-- 规则效果：自己场上的表侧表示怪兽不存在的场合或者只有「奏悦机组」怪兽的场合，这张卡可以从手牌特殊召唤。
	...
	c:RegisterEffect(e1)
	-- ①：这张卡在「奏悦机组」连接怪兽所连接区存在的场合，支付1000基本分，以对方场上最多2张卡为对象才能发动。那些卡和这张卡破坏。给予对方这个效果破坏的卡的数量×500的伤害。
...
-- 规则特召条件：自己场上有空余怪兽区域且不存在非「奏悦机组」的表侧表示怪兽
...
-- 破坏效果发动条件：检查此卡是否处于场上任意「奏悦机组」连接怪兽的所连接区
function s.descon(e,tp,eg,ep,ev,re,r,rp)
	...
	-- 遍历所有「奏悦机组」连接怪兽并合并其所连接区集合
	for lc in aux.Next(lg) do
		lg2:Merge(lc:GetLinkedGroup())
	end
	return lg2 and lg2:IsContains(e:GetHandler())
end
-- 破坏效果发动Cost：支付1000基本分
function s.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- Cost检查：判断玩家是否拥有至少1000基本分
	if chk==0 then return Duel.CheckLPCost(tp,1000) end
	-- 扣除玩家1000基本分
	Duel.PayLPCost(tp,1000)
end
-- 破坏效果发动准备：选择对方场上最多2张卡为对象，并将自身一同加入破坏目标，设置破坏与伤害操作信息
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsControler(1-tp) end
	-- 发动条件检查：对方场上是否存在可选择的对象
	...
	-- 设置连锁操作信息：破坏选中的对方卡片及自身
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,#g,0,0)
	-- 设置连锁操作信息：给予对方破坏数量×500的伤害
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,#g*500)
end
-- 破坏效果处理：同时破坏选中的对方卡片与自身，并根据实际破坏卡片数量给予对方伤害
