...
-- 效果作用：设置cost标签为100，表示已支付费用。
function c5288597.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(100)
	if chk==0 then return true end
end
-- 效果作用：过滤场上满足条件的怪兽（必须是表侧表示、可送入墓地、且其种族和属性在墓地中存在符合条件的怪兽）。
...
-- 效果原文内容：①：把自己场上1只表侧表示怪兽送去墓地才能发动。和墓地的那只怪兽种族·属性相同而等级高1星的1只怪兽从卡组特殊召唤。
