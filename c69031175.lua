...
function c69031175.initial_effect(c)
	-- 同调召唤手续：名称包含「黑羽」的调整+1只以上调整以外的怪兽
...
-- 放置指示物效果发动条件：伤害步骤结束时，此卡攻击对方怪兽，且战斗目标没有楔指示物并可以放置指示物
function c69031175.ctcon(e,tp,eg,ep,ev,re,r,rp)
	-- 获取此次战斗被攻击的怪兽
	local atg=Duel.GetAttackTarget()
	-- 发动条件检查：在伤害步骤结束时自身攻击战斗目标，且目标仍在场、无楔指示物并能放置指示物
...
-- 攻守变0效果Cost：取除对方场上全部怪兽的楔指示物，并设为效果处理目标
function c69031175.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- Cost检查：对方场上是否存在带有楔指示物的怪兽
	...
	-- 将所有有楔指示物放置过的怪兽设置为连锁的效果对象
	Duel.SetTargetCard(g)
end
-- 攻守变0效果处理：使所有带有楔指示物放置过的怪兽攻击力与守备力直到回合结束变成0
function c69031175.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	while t do
		if t:IsRelateToEffect(e) then
			-- 有楔指示物放置过的全部怪兽的攻击力·守备力直到回合结束时变成0。
			local e1=Effect.CreateEffect(c)
			e1:SetType(EFFECT_TYPE_SINGLE)
