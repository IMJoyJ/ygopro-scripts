...
-- 过滤手牌或场上的怪兽，满足条件的怪兽需在手牌或正面表示、为怪兽卡、可作为除外的代价，并且存在满足条件的第二只怪兽
function c20862918.costfilter1(c,tp)
	return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost()
		-- 检查是否存在满足条件的第二只怪兽，其种族、属性与第一只怪兽相同，且卡名不同
...
-- 过滤手牌或场上的怪兽，满足条件的怪兽需在手牌或正面表示、为怪兽卡、可作为除外的代价，并且种族、属性与第一只怪兽相同，卡名与第一只怪兽不同，同时卡组中存在满足条件的检索怪兽
...
-- 判断是否满足发动条件并选择除外的两只怪兽，然后将它们除外，设置后续操作信息
function c20862918.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		if e:GetLabel()~=100 then return false end
		e:SetLabel(0)
		-- 判断是否满足发动条件，即是否存在于手牌或场上的满足条件的怪兽
