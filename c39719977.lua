...
-- 发动时点判定：确认此效果为魔法卡发动，且自己场上存在至少3只满足filter条件的怪兽（即存在3只同名通常怪兽且非衍生物）。
function c39719977.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:IsHasType(EFFECT_TYPE_ACTIVATE)
		-- 检索自己场上是否存在至少3只满足filter条件的怪兽，作为满足发动条件（3只同名通常怪兽）的判定。
		and Duel.IsExistingMatchingCard(c39719977.filter,tp,LOCATION_MZONE,0,3,nil,tp) end
end
-- 效果处理时，获取所有满足条件的同名通常怪兽组，并给其中的每只怪兽注册一个‘本回合可以进行直接攻击’的效果。
function c39719977.activate(e,tp,eg,ep,ev,re,r,rp)
	-- 获取自己场上所有满足filter条件的怪兽（表侧表示、通常怪兽、非衍生物且存在另外2只同名同类怪兽）作为赋予直接攻击的对象组。
