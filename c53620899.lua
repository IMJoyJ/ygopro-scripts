...
function c53620899.initial_effect(c)
	-- 效果原文内容：②：这张卡反转的场合才能发动。自己从卡组抽1张，那之后选1张手卡丢弃。
	...
	c:RegisterEffect(e1)
	-- 效果原文内容：①：自己主要阶段才能发动。这张卡变成里侧守备表示（1回合只有1次）。
...
-- 规则层面作用：检查玩家是否可以抽卡并设置连锁操作信息，包括抽卡和丢弃手卡。
function c53620899.target(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 规则层面作用：判断是否满足抽卡条件。
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) end
	-- 规则层面作用：设置连锁操作信息，表示将要进行抽卡效果。
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
	-- 规则层面作用：设置连锁操作信息，表示将要进行丢弃手卡效果。
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,tp,1)
end
-- 规则层面作用：执行抽卡和丢弃手卡的操作，若抽卡成功则继续处理丢弃手卡。
...
-- 规则层面作用：检查是否可以将怪兽变为里侧守备表示并注册标志位。
function c53620899.postg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 规则层面作用：设置连锁操作信息，表示将要改变怪兽表示形式。
	Duel.SetOperationInfo(0,CATEGORY_POSITION,c,1,0,0)
end
-- 规则层面作用：执行将怪兽变为里侧守备表示的操作。
function c53620899.posop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and c:IsFaceup() then
		-- 规则层面作用：将目标怪兽变为里侧守备表示。
