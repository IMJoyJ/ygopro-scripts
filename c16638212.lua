...
-- 过滤满足条件的场上怪兽，包括表侧表示、可作为除外费用、且有可用怪兽区。
function c16638212.spfilter(c,tp)
	-- 检查目标怪兽是否满足表侧表示、可除外、且该玩家场上存在可用怪兽区。
...
function c16638212.spcon(e,c)
	...
	-- 检查玩家场上是否存在满足条件的怪兽（表侧表示、可除外、有可用怪兽区）。
