...
function c5861892.sptg(e,tp,eg,ep,ev,re,r,rp,chk,c)
	...
	-- 从mg中选择至少3张卡片，并将其保存为sg。如果选择了卡片，则使其保持存活状态，并将sg作为e的标签对象返回true；否则返回false。
...
-- 定义特殊召唤操作函数spop，将标签对象g中的卡送去墓地并删除该组。
...
-- 定义触发条件函数thcon，检查是否满足回收效果的条件：硬币正面、与战斗有关联且目标怪兽在墓地。
...
-- 定义取对象目标函数thtg，选择要加入手牌的卡片。
function c5861892.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and chkc:IsAbleToHand() end
	-- 如果chk为真值，则返回所选卡是否位于墓地、属于当前玩家控制并且可以被加入手牌。如果chk为0，则检查是否存在满足条件的卡片。
...
-- 定义触发操作函数thop，将选定的目标卡送去手牌并确认。
function c5861892.thop(e,tp,eg,ep,ev,re,r,rp)
	-- 获取当前连锁中的第一个目标卡。
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		-- 如果tc与效果相关联，则将其送去持有者的手牌并进行确认。
		Duel.SendtoHand(tc,nil,REASON_EFFECT)
		-- 向对方玩家确认所回收的卡片。
		Duel.ConfirmCards(1-tp,tc)
	end
end
-- 定义无效/破坏条件函数negcon，检查是否满足无效和破坏效果的条件：目标是可取对象、连锁的目标包含这张卡、硬币反面且目标是怪兽效果/魔法/陷阱。
...
function c5861892.negtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if re:GetHandler():IsRelateToEffect(re) and re:GetHandler():IsDestructable() then
		-- 如果连锁的目标是可破坏的，则设置操作信息，表示要破坏该卡片。
		Duel.SetOperationInfo(0,CATEGORY_DESTROY,eg,1,0,0)
	end
end
-- 定义无效/破坏操作函数negop，执行无效和破坏效果。
function c5861892.negop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- 检查当前卡是否反面、攻击力是否小于1000、与效果是否相关联以及当前连锁是否为ev+1。
	...
	if not c:IsHasEffect(EFFECT_REVERSE_UPDATE) then
		-- 如果Duel.NegateActivation(ev)成功且连锁的目标与当前效果相关联，则执行破坏效果。
