...
-- 判断连锁是否满足发动条件，包括对方发动、效果可无效、为怪兽卡、且在手卡、墓地或除外区
function c31548215.condition(e,tp,eg,ep,ev,re,r,rp)
	-- 获取当前连锁的发动位置
	local loc=Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_LOCATION)
	-- 判断发动玩家为对方、连锁效果可无效、发动效果为怪兽类型、且发动位置在手卡、墓地或除外区
...
function c31548215.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置连锁操作信息为对对方造成2000伤害
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,2000)
end
-- 执行效果，使连锁效果无效并造成对方2000伤害
