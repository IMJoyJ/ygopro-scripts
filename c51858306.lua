...
-- 设置连锁操作信息，指定将要除外的卡片数量为1张，来自卡组。
function c51858306.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	-- 设置当前处理的连锁的操作信息为除外效果，目标是玩家tp的卡组中的1张卡。
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,tp,LOCATION_DECK)
end
-- 定义过滤函数，用于筛选满足条件的怪兽（龙族、7星以上、光或暗属性且可除外）。
...
function c51858306.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置当前处理的连锁的操作信息为回手牌效果，目标是特定的怪兽。
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,tc,1,0,0)
end
-- 执行效果处理，将之前除外的怪兽送入持有者的手卡，并确认对方可见。
function c51858306.thop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetLabelObject():GetLabelObject()
	-- 检查该怪兽是否曾被①的效果除外过，并判断能否将其送入手卡。
