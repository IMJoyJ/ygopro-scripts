...
function c47658964.initial_effect(c)
	-- 把自己墓地存在的名字带有「熔岩」的怪兽任意数量从游戏中除外发动。
...
function c47658964.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 将选中的卡片以正面表示形式从游戏中除外作为发动代价
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
-- 为这张卡发动而除外的怪兽数量的「熔岩衍生物」（炎族·炎·1星·攻/守0）在自己场上守备表示特殊召唤。
function c47658964.target(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 判断是否满足发动条件，即玩家场上存在可用的怪兽区域
...
-- 执行发动效果，检查是否满足特殊召唤条件并处理特殊召唤过程
function c47658964.activate(e,tp,eg,ep,ev,re,r,rp)
	-- 获取玩家场上可用的怪兽区域数量
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	-- 判断是否满足特殊召唤条件，包括场上空位不足或无法特殊召唤衍生物
