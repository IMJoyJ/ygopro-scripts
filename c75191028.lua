...
local s,id,o=GetID()
-- 初始化卡片效果：注册融合召唤手续、素材检测、融合召唤成功时根据恐龙族素材数量赋予持续/诱发效果、以及用「GMX」卡的效果翻卡时全场怪兽破坏效果
function s.initial_effect(c)
	c:EnableReviveLimit()
	-- 注册融合召唤手续：「GMX」怪兽1只+恐龙族怪兽2只以上
	...
	c:RegisterEffect(e0)
	-- 融合召唤成功的场合，检查融合素材中恐龙族怪兽的数量并赋予对应效果
...
-- 效果赋予处理：根据素材中恐龙族怪兽数量（≥3/≥4/≥5）注册对应的持续/诱发效果
...
-- 召唤/特召过滤条件：确认是否为对方召唤·特殊召唤怪兽
function s.cfilter(c,tp)
	return c:IsSummonPlayer(tp)
end
-- 基本分扣减条件：存在对方召唤·特殊召唤的怪兽
function s.reccon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.cfilter,1,nil,1-tp)
end
-- 基本分扣减处理：展示卡片提示并使对方失去800基本分
...
-- 破坏效果发动条件：由自己玩家的效果触发翻卡
function s.descon(e,tp,eg,ep,ev,re,r,rp)
	return rp==tp
end
-- 破坏效果发动准备：设置破坏场上其他所有怪兽的操作信息
