...
function c24175232.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	-- 破坏选中的自己卡片，并记录实际被破坏的数量
	local oc=Duel.Destroy(sg,REASON_EFFECT)
	if oc==0 then return end
	-- 中断当前效果，使之后的破坏处理与先前的破坏处理视为不同时处理
	Duel.BreakEffect()
	-- 提示玩家选择对方场上要破坏的卡片
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)  --"请选择要破坏的卡"
...
function c24175232.spr(e,tp,eg,ep,ev,re,r,rp)
	...
	-- 检查当前回合玩家是否为自己，且当前阶段是否为准备阶段
	if Duel.GetTurnPlayer()==tp and Duel.GetCurrentPhase()==PHASE_STANDBY then
		-- 将当前回合数存入Label，用于在下次自己准备阶段时判定
...
function c24175232.spcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	-- 检查自身是否可以特殊召唤、是否不是在被破坏的当回合、当前是否为自己回合，且是否存在复活标记
...
function c24175232.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		-- 以及自己场上是否有可用于特殊召唤怪兽的可用怪兽区域
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
	-- 设置效果分类为特殊召唤，预计将自身特殊召唤
