...
-- 定义过滤函数，用于检测场上是否存在表侧表示的调整怪兽。
function c23571046.cfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_TUNER)
end
-- 判断是否满足发动条件，即自己场上是否存在至少1只调整怪兽。
