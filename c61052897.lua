...
local s,id,o=GetID()
-- 初始化卡片效果：注册手卡特召并同调诱发即时效果、自定义同调素材等级效果
...
-- 检查当前选取的素材组合等级合计是否符合同调怪兽等级
function s.syngoal(g,tp,lv,syncard,minc,ct)
	-- 检查素材数量是否满足下限且额外区域有可用空格
...
-- 发动条件：对方主要阶段且自己场上有「次世代」怪兽存在
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	-- 获取当前阶段
	local ph=Duel.GetCurrentPhase()
	-- 判断是否在对方主要阶段1或2且场上存在表侧表示「次世代」怪兽
...
-- 效果处理：将这张卡特殊召唤，随后可进行「次世代」同调怪兽的同调召唤
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)>0 then
		-- 刷新场地状态
		Duel.AdjustAll()
		-- 检查是否存在可同调召唤的「次世代」怪兽并询问是否进行同调召唤
