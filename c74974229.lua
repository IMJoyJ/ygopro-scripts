...
function c74974229.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- ②：这张卡被战斗·效果破坏送去墓地的场合才能发动。从自己墓地选最多有有着A指示物放置的对方场上的怪兽数量的连接怪兽以外的爬虫类族怪兽特殊召唤（同名卡最多1张）。
...
-- ①效果发动准备：检查场上可放置A指示物的怪兽并设置操作信息
function c74974229.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查双方场上是否存在可以放置A指示物(0x100e)的表侧表示怪兽
...
-- ①效果处理：把丢弃怪兽原本等级数量的A指示物任意分配给场上的表侧表示怪兽
...
-- ②效果发动准备：检查怪兽区空位、墓地目标怪兽及对方场上有A指示物的怪兽，并设置特召操作信息
function c74974229.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查自己怪兽区域是否有空位
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		-- 检查墓地是否存在可特殊召唤的非连接爬虫类族怪兽
...
-- ②效果处理：从墓地选最多有有A指示物的对方怪兽数量的非连接爬虫类族怪兽表侧表示特召（同名最多1张）
function c74974229.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	-- 选择1到数量上限（受空位数与指示物怪兽数限制）且互不同名的怪兽
