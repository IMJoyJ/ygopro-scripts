...
function c94807487.initial_effect(c)
	...
	-- 卡片发动与装备选择：以场上1只表侧表示的「希望皇 霍普」怪兽为对象发动并装备
	...
	c:RegisterEffect(e3)
	-- ②（放置指示物）：只要这张卡在魔法与陷阱区域存在，每次怪兽的攻击无效，给这张卡放置1个希望剑指示物。
	...
	c:RegisterEffect(e4)
	-- ②（攻击力上升）：装备怪兽的攻击力上升这张卡的希望剑指示物数量×500。
	local e5=Effect.CreateEffect(c)
	e5:SetType(EFFECT_TYPE_EQUIP)
	e5:SetCode(EFFECT_UPDATE_ATTACK)
	e5:SetValue(c94807487.value)
	c:RegisterEffect(e5)
	-- ③：自己场上的装备怪兽把超量素材去除来让效果发动的场合，这张卡可以当作去除的超量素材中的1个使用。
