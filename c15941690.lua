...
-- 判断特殊召唤条件是否满足，包括自己场上没有怪兽、对方场上存在怪兽、且自己场上存在可用召唤区域。
...
-- 判断是否满足特殊召唤目标的条件，包括场上存在可用区域和满足条件的怪兽。
function c15941690.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置连锁操作信息，表示将要特殊召唤1只怪兽，目标为手卡或墓地。
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_GRAVE+LOCATION_HAND)
end
-- 执行特殊召唤流程，包括检查召唤区域、提示选择、选择目标怪兽并进行特殊召唤。
function c15941690.spop(e,tp,eg,ep,ev,re,r,rp)
	-- 检查场上是否还有可用召唤区域，若无则直接返回。
