...
function c19870120.initial_effect(c)
	...
	c:RegisterEffect(e2)
	-- ①：只要这张卡在魔法与陷阱区域存在，自己场上的上级召唤的怪兽不会成为效果的对象。
	...
	e3:SetValue(1)
	c:RegisterEffect(e3)
	-- ①：只要这张卡在魔法与陷阱区域存在，自己场上的上级召唤的怪兽不会被效果破坏。
...
-- 判定尝试特殊召唤的怪兽是否位于额外卡组，若是则禁止该特殊召唤，从而实现不能从额外卡组特殊召唤怪兽的限制。
function c19870120.sumlimit(e,c,sump,sumtype,sumpos,targetp)
	return c:IsLocation(LOCATION_EXTRA)
end
-- 判定卡片是否是以上级召唤方式被召唤的怪兽，用于筛选出受保护的我方场上上级召唤的怪兽。
