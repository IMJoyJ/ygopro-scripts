...
function c69456283.initial_effect(c)
	-- 1回合只有1次，在自己的主要阶段，场上的这张卡可以当作装备卡使用给「彼者」装备
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(69456283,0))  --"装备"
	...
	e2:SetDescription(aux.Stringid(69456283,1))  --"特殊召唤"
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_SZONE)
	-- 设置效果发动条件为：自身处于同盟装备状态
	...
	e3:SetValue(3000)
	-- 设置效果适用条件为：自身处于同盟装备状态
	e3:SetCondition(aux.IsUnionState)
	c:RegisterEffect(e3)
	-- 装备怪兽攻击守备表示的怪兽时，若攻击力超过那个守备力，给与对方基本分那个数值的战斗伤害。
