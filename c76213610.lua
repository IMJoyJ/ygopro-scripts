...
local s,id,o=GetID()
-- 初始化卡片效果：注册超量召唤手续、①主要阶段去除素材特召墓地不死族并可无效对方怪兽效果、②被送去墓地洗回墓地怪兽效果
function s.initial_effect(c)
	-- 设置超量召唤手续：6星怪兽×2
	aux.AddXyzProcedure(c,nil,6,2)
	c:EnableReviveLimit()
	-- ①：主要阶段可以把这张卡1个超量素材去除，以自己墓地1只不死族怪兽为对象才能发动。那只怪兽特殊召唤。那之后，可以把持有那只怪兽攻击力以下攻击力的对方场上1只怪兽的效果无效化。
...
-- 特召过滤条件：墓地的不死族怪兽且可特殊召唤
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_ZOMBIE) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
-- ①效果发动准备：选择墓地1只不死族怪兽为对象并设置特召操作信息
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and s.spfilter(chkc,e,tp) end
	-- 发动条件检查：怪兽区域有空位
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		-- 发动条件检查：墓地是否存在可特召的不死族怪兽
