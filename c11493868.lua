...
function c11493868.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	-- 检查是否满足效果发动条件，即场上是否存在1只表侧表示的「幻奏」怪兽
...
-- 过滤函数，用于判断是否为在场上的怪兽且未被效果免疫
function c11493868.filter1(c,e)
	return c:IsOnField() and not c:IsImmuneToEffect(e)
end
-- 过滤函数，用于判断是否为「幻奏」融合怪兽且满足融合召唤条件
...
-- 效果处理时的确认阶段，检查是否存在可融合召唤的「幻奏」融合怪兽
function c11493868.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=tp
		-- 获取玩家当前可用的融合素材，并筛选出在场上的怪兽
		...
		if not res then
			...
			if ce~=nil then
				...
				-- 若存在连锁融合素材效果，则检查其是否能融合召唤出满足条件的怪兽
...
function c11493868.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	-- 获取玩家当前可用的融合素材，并筛选出在场上的怪兽且未被效果免疫
