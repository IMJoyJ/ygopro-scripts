...
function c28429121.initial_effect(c)
	-- 将此卡注册为一个可以发动的魔法卡，具有特殊召唤的分类，可以在自由连锁时发动，需要支付费用，目标为自身，发动时执行activate效果
...
function c28429121.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	-- 检查当前阶段是否不是主要阶段2，若是则返回true表示可以发动此卡
...
function c28429121.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置操作信息，表示本次效果处理将特殊召唤1张手牌中的怪兽
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
-- 执行仪式召唤的处理流程，包括选择仪式怪兽、选择解放的素材、设置攻击力减半效果并特殊召唤
function c28429121.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		-- 设置额外的仪式召唤检查函数，用于验证所选素材是否满足等级要求
		aux.GCheckAdditional=aux.RitualCheckAdditional(tc,tc:GetLevel(),"Equal")
		-- 从可用素材中选择满足等级要求的子集作为解放的素材
