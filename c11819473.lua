...
function c11819473.initial_effect(c)
	...
	-- 这个卡名的①②的效果1回合各能使用1次。①：进行1次投掷硬币，那个里表的以下效果适用。自己的场地区域有「光之结界」存在的场合，不进行投掷硬币而选里表的其中1个适用。●表：从卡组把「秘仪读牌」以外的1张持有进行投掷硬币效果的卡加入手卡。●里：对方从自身卡组选1张卡加入手卡。
...
-- 过滤条件：非同名且持有投币效果且可以加入手卡
function c11819473.thfilter1(c)
	-- 卡名不是「秘仪读牌」且持有投掷硬币效果且可以加入手卡
