...
function c27642961.initial_effect(c)
	...
	-- 注册一个在自己结束阶段触发的持续效果，用于记录回合数并在第3次结束阶段时破坏此卡
...
function c27642961.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- 设置一个在自己结束阶段触发的持续效果，用于记录回合数并在第3次结束阶段时破坏此卡
	local e1=Effect.CreateEffect(c)
...
function c27642961.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	c:SetTurnCounter(ct)
	if ct==3 then
		-- 在第3次结束阶段时以REASON_RULE原因破坏此卡
		Duel.Destroy(c,REASON_RULE)
	end
end
-- 过滤函数，用于判断被破坏的卡是否来自场上且为效果破坏
function c27642961.cfilter(c,tp)
	return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsReason(REASON_EFFECT)
end
-- 判断是否满足①效果的发动条件，即自己的「秘旋谍」怪兽战斗破坏或效果破坏
...
function c27642961.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	-- 让目标玩家以REASON_EFFECT原因抽卡
	Duel.Draw(p,d,REASON_EFFECT)
end
-- 过滤函数，用于筛选手卡中的「秘旋谍」怪兽
